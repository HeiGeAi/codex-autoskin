...
function Resolve-CodexStoreExecutablePath {
  ...
  if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
    throw "Codex executable not found: $executable"
  }
  return ConvertTo-NormalizedProcessPath -LiteralPath $executable
}

function Get-ProcessesByExecutablePath {
  param(
    [Parameter(Mandatory)][string]$ExpectedExecutablePath,
    [Parameter(Mandatory)][AllowEmptyCollection()][object[]]$CandidateProcesses
  )

  $expected = ConvertTo-NormalizedProcessPath -LiteralPath $ExpectedExecutablePath
  foreach ($process in $CandidateProcesses) {
    if ($null -eq $process) { continue }
    try {
      $actual = ConvertTo-NormalizedProcessPath -LiteralPath ([string]$process.Path)
    } catch {
      continue
    }
...
function Get-CodexProcesses {
  param(
    [Parameter(Mandatory)][string]$ExpectedExecutablePath,
    [AllowEmptyCollection()][object[]]$CandidateProcesses
  )

  if (-not $PSBoundParameters.ContainsKey('CandidateProcesses')) {
    $CandidateProcesses = @(Get-Process -Name ChatGPT -ErrorAction SilentlyContinue)
  }
  return @(Get-ProcessesByExecutablePath -ExpectedExecutablePath $ExpectedExecutablePath -CandidateProcesses $CandidateProcesses)
}
