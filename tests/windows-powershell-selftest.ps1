...
'@
  ...
  Invoke-SelfTest 'same-name ChatGPT processes are selected only by the Codex Store executable path' {
    $expectedExecutable = Join-Path $Sandbox 'store\OpenAI.Codex\app\ChatGPT.exe'
    $otherExecutable = Join-Path $Sandbox 'other-app\ChatGPT.exe'
    $codexProcess = [pscustomobject]@{
      Id = 4101
      ProcessName = 'ChatGPT'
    ...
    $otherProcess = [pscustomobject]@{
      Id = 4102
      ProcessName = 'ChatGPT'
      Path = $otherExecutable
      MainWindowHandle = [IntPtr]2
    }

    $selected = @(Get-CodexProcesses -ExpectedExecutablePath $expectedExecutable -CandidateProcesses @($codexProcess, $otherProcess))
    Assert-Equal -Expected 1 -Actual $selected.Count -Message 'A same-name process from another executable path must be ignored.'
    Assert-Equal -Expected $codexProcess.Id -Actual $selected[0].Id -Message 'The resolved Codex Store executable must be selected.'
  }
