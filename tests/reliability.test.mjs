...
test("same-name ChatGPT processes are filtered by the resolved Codex Store executable", async () => {
  const helper = await fs.readFile(path.join(repoRoot, "scripts", "process-ownership.ps1"), "utf8");
  const start = (await fs.readFile(path.join(repoRoot, "scripts", "start-dream-skin.ps1"), "utf8")).replace(/\r\n/g, "\n");
  const watcher = (await fs.readFile(path.join(repoRoot, "scripts", "watch-dream-skin.ps1"), "utf8")).replace(/\r\n/g, "\n");

  assert.match(helper, /function Resolve-CodexStoreExecutablePath/);
  assert.match(helper, /function Get-CodexProcesses/);
  assert.match(helper, /Get-ProcessesByExecutablePath/);
  assert.match(helper, /\$CandidateProcesses\s*=\s*@\(Get-Process -Name ChatGPT[^\n]+\)/);
  assert.match(helper, /Get-ProcessesByExecutablePath -ExpectedExecutablePath \$ExpectedExecutablePath -CandidateProcesses \$CandidateProcesses/);

  for (const [name, source] of [["launcher", start], ["watcher", watcher]]) {
    assert.match(source, /Resolve-CodexStoreExecutablePath/, `${name} must resolve the Store executable`);
    assert.match(source, /Get-CodexProcesses[^\n]+ExpectedExecutablePath/, `${name} must filter by that path`);
    assert.doesNotMatch(source, /Get-Process\s+(?:-Name\s+)?ChatGPT/, `${name} must not enumerate by name directly`);
  }

  const stopStart = start.indexOf("function Stop-CodexCompletely");
  const stopEnd = start.indexOf("\n}\n\n$CodexExecutablePath", stopStart);
  assert.ok(stopStart >= 0 && stopEnd > stopStart, "the launcher must contain one bounded stop function");
  const stopFunction = start.slice(stopStart, stopEnd);
  assert.equal(
    [...stopFunction.matchAll(/Get-CodexProcesses -ExpectedExecutablePath \$ExpectedExecutablePath/g)].length,
    3,
    "every graceful-close, force-stop, and respawn sweep must query only the expected executable",
  );
  assert.match(stopFunction, /\$visible\s*=\s*@\(Get-CodexProcesses[^\n]+\| Where-Object[^\n]+MainWindowHandle/);
  const closeLines = stopFunction.split("\n").filter((line) => line.includes("CloseMainWindow"));
  assert.deepEqual(closeLines, ["  foreach ($process in $visible) { [void]$process.CloseMainWindow() }"]);
  const stopLines = stopFunction.split(/\r?\n/).filter((line) => line.includes("Stop-Process"));
  assert.equal(stopLines.length, 2);
  for (const line of stopLines) {
    assert.match(line, /(?:\$procs|Get-CodexProcesses[^|]+)\s*\|\s*Stop-Process -Force/);
    assert.doesNotMatch(line, /-(?:Name|Id)\b/);
  }

  assert.match(
    watcher,
    /\$mainProcesses\s*=\s*@\(Get-CodexProcesses -ExpectedExecutablePath \$CodexExecutablePath \| Where-Object[^\n]+MainWindowHandle/,
  );
  assert.match(watcher, /& \$StartScript -Port \$Port -RestartExisting/);
});
